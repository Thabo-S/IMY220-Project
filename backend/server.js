...
app.post('/api/projects', upload.fields([{ name: 'files', maxCount: 10 }, { name: 'image', maxCount: 1 }]), async (req, res) => {
    try {
        ...
        const newProject = {
            ...
            name, description, hashtag, isPublic: isPublic === 'true',
            creatorId: parseInt(creatorId), createdAt: new Date().toISOString(),
            downloads: 0, lastActivity: new Date().toISOString(),
            files, image, messages: [], lockedBy: null
        };
...
app.post('/api/projects', upload.fields([{ name: 'files', maxCount: 10 }, { name: 'image', maxCount: 1 }]), async (req, res) => {
    try {
        ...
        const newProject = {
            name,
            description,
            hashtags: hashtag ? hashtag.split(',').map(tag => tag.trim().replace('#', '')) : [],
            files,
            image,
        ...
        res.status(201).json({ ...newProject, id: result.insertedId });
    } catch (error) {
...
app.put('/api/projects/:id', upload.fields([{ name: 'files', maxCount: 10 }, { name: 'image', maxCount: 1 }]), async (req, res) => {
    try {
        ...
        const image = req.files['image'] ? `/uploads/${req.files['image'][0].filename}` : null;
        const update = { name, description, hashtag, isPublic: isPublic === 'true', lastActivity: `User updated project at ${new Date().toLocaleString("en-US", { timeZone: "Africa/Johannesburg" })}` };
...
app.post('/api/projects/:id/checkin', upload.array('files', 10), async (req, res) => {
    try {
        ...
        const message = { id: Date.now(), userId: parseInt(userId), text, time: new Date().toISOString() };
        const update = { $push: { messages: message }, lastActivity: message.time };
        if (files.length) update.$push.files = { $each: files };
        const result = await db.collection("projects").updateOne({ id: projectId, lockedBy: userId }, { $set: update });
        if (result.matchedCount === 0) return res.status(403).json({ message: "Project not checked out by you" });
        const updatedProject = await db.collection("projects").findOne({ id: projectId });
        res.json(updatedProject);
    } catch (error) {
        res.status(500).json({ message: "Server error" });
...
app.get('/api/activity/local/:userId', async (req, res) => {
  try {
    ...
    // Get the user details for those creators
...
app.get('/api/activity/global', async (req, res) => {
    try {
        ...
        // Get all public projects with messages
